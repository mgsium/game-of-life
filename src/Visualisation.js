...
export class Visualisation {
    ...
    Simulate = async () => {
        ...
        while (this.houses.length > 0 && this.isSimulating){
            ...
            y_vals.forEach(num => {
                if (num < lowest_y) lowest_y=num;
                if (num > highest_y) highest_y=num;
            });
            ...
            x_vals.forEach(num => {
                if (num < lowest_x) lowest_x=num;
                if (num > highest_x) highest_x=num;
            });
    ...
    checkNeighbours = async (placesToCheck) => {
        ...
        for (let i = 0; i < len; i++){
            ...
            // checks each adjacent block for the prescence of a house
            [
                [1, 0],
                [1, 1],
                [0, 1],
                ...
                if (this.houses.includes(`${parseInt(y, 10)+a}-${parseInt(x, 10)+b}`)) {
                    neighbours++;
                }
            });
            ...
            } else if(neighbours === 3) newHouseList.push(id);
        }
